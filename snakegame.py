...
class snake:
    def __init__(self,x,y):
        ...
        self.y = y
        self.direction = KEY["UP"]
        self.stack = []  #intially it will be empty
...
#Define the moves of the snake
    ...
    #Makes the snake grow when it eats
    def grow(self):
        last_element = len(self.stack) - 1
        self.stack[last_element].direction = self.stack[last_element].direction
    ...
    def get_rect(self): #Gets the rectangle shape
        rect = (self.x, self.y)
        return rect
    ...
    #Funtion for when the snake eats it's self
...
def respawnApple(apples, index, sx, sy):
    ...
    angle = 999
    while(angle > radius):
        angle = random.uniform(0, 999) * math.pi * 2
...
def main():
    ...
    mySnake.move()
    start_segments = 3 #Three segment long snake 
    ...
    #Food
    max_apples = 1 #one apple at a time
    eaten_apple = False
    apples = [Apple(random.randint(60, SCREEN_WIDTH), random.randint(60, SCREEN_HEIGHT), 1)]
    respawnApples(apples, max_apples, mySnake, mySnake.y)
